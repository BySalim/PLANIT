...
| ~~TD-LH-CSP~~      | ~~Audit Lighthouse `csp-xss` à 0 — pas de Content Security Policy sur l'app web~~                        | **Résolu** (2026-05-31, pré-release V02) : CSP modérée posée via `next.config.ts > headers()` (default-src 'self', script/style en `'unsafe-inline'` car Next.js hydration, object-src 'none', frame-ancestors 'none', base-uri 'self'). Hardening nonce-based reporté au sprint sécurité prod                                                                              | —        |
| ~~TD-LH-CONSOLE~~  | ~~Audit Lighthouse `errors-in-console` à 0 — des erreurs JS sont logguées au chargement~~                | **Résolu** (2026-05-31, pré-release V02) : `useWeekSessionsQuery`, `useSessionDetailQuery`, `useWeekStatsQuery` et `useRealtimeSessions` gate désormais sur `state.status === 'authenticated'` du `AuthContext`. Plus de fetch/socket sortant avant que `<RequireAuth>` ait redirigé — supprime les `connection refused` que LH CI capte (backend pas lancé dans le job LH) | —        |
...
| TD-LH-WEIGHT       | Audit Lighthouse `total-byte-weight` à 0 — payload mobile > seuil par défaut (~1.6 MB)                   | **Non bloquant en V02** : l'audit n'apparaît plus failing dans les runs récents (First Load JS ~175 kB sur /etudiant et /enseignant après build). À surveiller en V03 si le bundle grossit ; quick wins prêts (`dynamic()` sur modals/calendar) si besoin                                                                                                                   | Faible   |
