...
| TD-LH-LANDMARK | Audit Lighthouse `landmark-one-main` à 0 — pas de balise `<main>` dans le DOM              | Accessibilité semantic HTML (WCAG 2.1 A). Le shell mobile utilise `<div>` au lieu de `<main role="main">`. À fixer dans LOT 7 D.4 (sprint responsive) ou avant — c'est 5 min de modif sur `mobile-shell.tsx` + layouts                                                       | Moyenne  |
