...
| TD-V02-LOT3-A | Pas d'endpoint `GET /api/classes` côté backend (référentiel hardcodé frontend dans `queries-v2.ts`) | LOT 3 R.3 — `<ClasseChipsPicker>` lit une liste statique de 3 classes seed. Backend manquant à créer par Salim/Oumar (controller + service lecture seule). Spec V02-01 annonçait "déjà disponible V01" — faux | Haute    |
...
| TD-V02-LOT3-D | Pas d'endpoint `GET /api/salles` côté backend (select Salle reste vide dans modal + drawer V2)      | UX dégradée — RP ne peut pas affecter de salle aux séances depuis le formulaire ; `salleId` reste null à la création                                                                                          | Moyenne  |
