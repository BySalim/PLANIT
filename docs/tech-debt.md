...
| TD-LH-CSP     | Audit Lighthouse `csp-xss` à 0 — pas de Content Security Policy sur l'app web              | Pré-existant, sans rapport avec une PR particulière. À fixer via Helmet + headers Next.js (`next.config.ts > headers()`) au sprint sécurité prod (avant le déploiement Hetzner). Référencé dans l'audit sécurité interne                                                     | Moyenne  |
| TD-LH-CONSOLE | Audit Lighthouse `errors-in-console` à 0 — des erreurs JS sont logguées au chargement      | Très probablement les `fetch` 401 qui partent au mount avant que `<RequireAuth>` ait redirigé. Solutions : `enabled: isAuthenticated` sur les TanStack queries, ou catch silencieux dans `apiGet` pour les 401 attendus. À investiguer via le rapport Lighthouse HTML        | Moyenne  |
| TD-LH-WEIGHT  | Audit Lighthouse `total-byte-weight` à 0 — payload mobile > seuil par défaut (~1.6 MB)     | Bundle Next + React 19 + Tailwind v4 + design tokens + libs. Pas critique pour une app interne sur campus mais à profiler avec `next build` output. Possibles : code-splitting routes-based, tree-shaking design-tokens, lazy-loading shadcn. Sprint optim perf à programmer | Moyenne  |
