...
## Patterns émergés Vague 05 — LOT 7 (planning par référentiel fidèle PLANIT-IA + Année Direction)

> Capitalisation du LOT 7 (suite du LOT 6, retours TL). Acquis : tout nouveau code planning suit ces conventions.

- **Onglets planning RP = `Classique · Classe · Salle · Prof`** (réf. PLANIT-IA, annule « Mon espace » du LOT 6). **Classique** = un référentiel unique vu en **semaine** via `<PlanningGrid>` ; le choix passe par [referentiel-combobox.tsx](apps/web/src/components/planning/referentiel-combobox.tsx) (bouton + popover, sous-onglets Classe/Salle/Prof, recherche, liste groupée ; options `useClassesQuery`/`useSallesQuery`/`useEnseignantsQuery` déjà `isRp`-gated ; défaut = 1ʳᵉ classe possédée). **Classe/Salle/Prof** = vues **mono-jour multi-colonnes** ([planning-grid-by-entity.tsx](apps/web/src/components/planning/planning-grid-by-entity.tsx)) pilotées par [day-select.tsx](apps/web/src/components/planning/day-select.tsx).
- **`PlanningGridByEntity` = composant frère de `PlanningGrid`, pas une généralisation** : il **réutilise l'axe temps** (mêmes constantes `HOUR_HEIGHT`/snap/clamp, `ResizeHandle`, `SessionCard` y compris la branche `masked`) mais les **colonnes sont des entités** (classes/salles/enseignants) sur **un seul jour**. Éditable : déplacement vertical → heure ; **drop cross-colonne → réaffecte la dimension** (`classeIds=[col]` / `salleId=col` / `enseignantId=col` via `useUpdateSessionV2Mutation`) ; resize ; create sur cellule vide préremplit l'entité de la colonne (`CreateSessionModal.initialValues` étendu `salleId`/`enseignantId`). Cartes `session.masked` **non interactives**. Lecture seule AC/Direction via `readOnly`. Ne pas fusionner les deux grilles (axes de colonnes orthogonaux jours vs entités) — frères assumés.
- **`scope=ecole` (vue byroom) réutilise le masquage LOT 6, pas un nouvel endpoint** : `seance-v2.service.findWeek` traite `salleId` (référentiel Salle) **et** `scope=ecole` (vue Salle by-X) par la **même** branche `(query.salleId || query.scope === 'ecole') && isRp(user.role)` → `toMaskedSessionV2Dto` pour `ownerRpId !== user.id`. Le where ajoute `seanceClasses.some(ecoleClasseFilter(ecoleId))` (occupation école entière). Détails jamais sérialisés. **Test d'assertion négative obligatoire** (`lot6-isolation.spec` : `masked=true` + `module/classes/enseignant` nuls). Seule exception à l'isolation RP, comme le référentiel Salle.
