...
- **Pattern auth frontend complet** ([auth-context.tsx](apps/web/src/contexts/auth-context.tsx) + [require-auth.tsx](apps/web/src/components/auth/require-auth.tsx) + [api.ts](apps/web/src/lib/api.ts) + [forbidden-listener.tsx](apps/web/src/components/auth/forbidden-listener.tsx)) : `<AuthProvider>` dans root layout boot via `fetch('/auth/me')` (avec AbortController au cleanup, cf. déjà documenté), `useAuth()` retourne `{ state, login, logout }` où `state.status ∈ {loading, authenticated, unauthenticated}`. `<RequireAuth roles={...}>` wrappe chaque layout d'acteur (`/rp`, `/enseignant`, `/etudiant`) et redirige vers `/login` ou la home du rôle réel. Intercepteur 401 dans `request()` ([api.ts](apps/web/src/lib/api.ts)) tente UN refresh atomique (singleton `pendingRefresh`) puis rejoue ; échec → `window.location.href = '/login'`. 403 → `CustomEvent('api:forbidden')` capté par `<ForbiddenListener>` qui flash « Accès refusé ». Cookie HttpOnly = pas accessible JS → pas de check "suis-je auth ?" côté client autre que `state.status` du contexte.
