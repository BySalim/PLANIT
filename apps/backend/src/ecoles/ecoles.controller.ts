...
import { createEcoleSchema, updateEcoleSchema } from '@planit/contracts';
import type { CreateEcoleDto, EcoleDto, UpdateEcoleDto } from '@planit/contracts';
...
export class EcolesController {
  ...
  @ApiOperation({ summary: 'Créer une école et sa Direction (indissociables)' })
  @ApiResponse({ status: 201, description: 'École + Direction créées' })
  @ApiResponse({ status: 409, description: 'Nom d’école ou email Direction déjà pris' })
  create(
